...
function createAdmin($request_values){
	...
	if (empty($username)) { array_push($errors, "Uhmm... username manquant"); }
	if (empty($email)) { array_push($errors, "Oops.. Email manquant"); }
	if (empty($role)) { array_push($errors, "Role manquant");}
	if (empty($password)) { array_push($errors, "uh-oh mots de passe maquant"); }
	if ($password != $passwordConfirmation) { array_push($errors, "les mots de passe ne correspondent pas"); }
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Admin modifier";
		header('location: users.php');
		exit(0);
	}
}
...
function createTopic($request_values){
	...
	if (mysqli_num_rows($result) > 0) { 
		array_push($errors, "Topic  exists");
	}
